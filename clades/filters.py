...
from clades.models import Kingdom, Phylum, Classe, Order, Family, Genus, Species 

class CladisticFilter(SimpleListFilter):
    title = _('Cladistic filter')
    parameter_name = 'clade_filter'
    hierarchy = ['Kingdom', 'Phylum', 'Classe', 'Order', 'Family', 'Genus','Species']

    def __init__(self, request, params, model, model_admin):
        super(CladisticFilter, self).__init__(request, params, model, model_admin)
        ...
        print "ClFl model: {0}".format(model)
        print "ClFl hierarchy: {0}".format(self.hierarchy)
