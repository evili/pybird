...
class Lens(BaseNamed):
    brand        = models.ForeignKey(Brand)
    flength_min  = models.IntegerField(validators=[MinValueValidator(1),MaxValueValidator(1000)])
    flength_max  = models.IntegerField(validators=[MinValueValidator(1),MaxValueValidator(1000)])
    aperture_min = models.FloatField(validators=[MinValueValidator(1),MaxValueValidator(1000)])
    aperture_max = models.FloatField(validators=[MinValueValidator(1),MaxValueValidator(1000)])
    macro        = models.BooleanField(default=False)

    def clean(self):
        mina,maxa = self.aperture_min,self.aperture_max
        if (mina  > maxa):
            raise ValidationError('Max Aperture '+str(maxa)+' lower than Min Aperture '+str(mina))

        minf,maxf = self.flength_min,self.flength_max
        if (minf  > maxf):
            raise ValidationError('Max Focal '+str(maxa)+' lower than Min Focal '+str(mina))
...
class Roll(BaseNamed):
    ...
    iso   = models.IntegerField(blank=True, null=True,validators=[MinValueValidator(1),MaxValueValidator(1000)])
